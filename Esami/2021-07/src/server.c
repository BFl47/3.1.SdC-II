...
int main(int argc, char *argv[])
{
    ...
    if (ret) handle_error("main: errore bind");
	ret = listen(listening_socket_desc, MAX_CONN_QUEUE);
    if (ret) handle_error("main: errore listen");

    printf("effettuate bind, listen\n");
    ...
    while (1) {
        ...
        pid = fork();
        if (pid == -1) 
            handle_error("main: errore fork");
        else if (pid == 0) {
            ret = close(listening_socket_desc);
            if (ret) handle_error("main: close listening_socket_desc");

            connection_handler(client_desc, (struct sockaddr_in*) &client_addr);

            ret = close(client_desc);
            if (ret) handle_error("main: close client_desc");
            _exit(0);
        } else {
            ret = close(client_desc);
            if (ret) handle_error("main: close client_desc");
