...
unsigned int process_resource(unsigned int client_id, unsigned int resource_id) {
    ...
    ret = sem_wait(&semafori[resource_id]);
...
void* connection_handler(void* arg) {
    ...
    while (1) {
        ...
        do {
            ...
            if (ret == -1) {
                ...
                handle_error("connection_handler errore recv");
            }
            if (ret == 0) break;
            bytes_read += ret;
        ...
        while (bytes_sent < msg_len) {
            ...
            if (ret == -1) {
                if (errno == EINTR) continue;
                handle_error("connection_handler errore send");
            }
            bytes_sent += ret;
        }
    }
    ...
    if (ret != 0) handle_error("connection_handler errore close");
    free(args);
...
int main(int argc, char* argv[]) {	  	
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (ret == -1) handle_error("main errore sem_init");
    }
    ...
    while (1) { 
        ...
        if (ret != 0) handle_error_en(ret, "main errore pthread_create");
