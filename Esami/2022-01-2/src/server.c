 ...
 * 
 */
sem_t sem_full, sem_empty, sem_cs;
...
void init_socket(){
    ...
    if (socket_desc == -1) handle_error("init_socket: socket");

    ret = bind(socket_desc, (struct sockaddr*) &server_addr, (socklen_t) sockaddr_len);
    if (ret) handle_error("init_socket: bind");

    ret = listen(socket_desc, MAX_CONN_QUEUE);
    if (ret) handle_error("init_socket: listen");
...
void *thread_connection_handler(void *arg) {
    ...
    while (1) {
        ...
        if (m.type == 'w'){
            ...
            if (ret) handle_error("thread_conn_handler: sem_wait empty");

            ret = sem_wait(&sem_cs);
            if (ret) handle_error("thread_conn_handler: sem_wait cs");
            ...
            ret = sem_post(&sem_cs);
            if (ret) handle_error("thread_conn_handler: sem_post cs");

            ret = sem_post(&sem_full);
            if (ret) handle_error("thread_conn_handler: sem_post full");
        }
        if (m.type == 'r'){
            printf("received command %c\n",m.type);
            fflush(stdout);
            ...
            if (ret) handle_error("thread_conn_handler: sem_wait full");
    ...
    free(args);
    ret = close(sock_desk);
    if (ret) handle_error("thread_conn_handler: close sock_desk");
...
void mthreadServer() {
    ...
    while (1) {
        ...
        if (client_desc == -1) handle_error("mthreadServer: accept");
...
void close_everything() {
    ...
    if (ret) handle_error("close_everything: close socket_desc");
