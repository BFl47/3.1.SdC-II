...
int write_to_pipe(int fd, const void *data, size_t data_len) {
    ...
    int written = 0;
    while (data_len > 0) {
        int ret = write(fd, data + written, data_len);
...
int read_from_pipe(int fd, void *data, size_t data_len) {
    ...
    int read_bytes = 0;
    while (data_len > 0) {
        int ret = read(fd, data + read_bytes, data_len);
...
void reader(int reader_id, sem_t* read_mutex) {
     ...
     **/
    if (close(pipefd[1])) handle_error("reader: close pipefd[1]"); 
...
void writer(int writer_id, sem_t* write_mutex) {
    ...
    if (close(pipefd[0])) handle_error("writer: close pipefd[0]");
     ...
     * - gestire eventuali errori
     **/
    if (close(pipefd[1])) handle_error("writer: close pipefd[1]");     
...
int main(int argc, char* argv[]) {
     ...
     * - gestire eventuali errori
     **/
    if (pipe(pipefd)) handle_error("main: error creating pipe");
