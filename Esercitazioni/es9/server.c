...
unsigned int process_resource(unsigned int client_id, unsigned int resource_id) {
     ...
     **/
    int ret = sem_wait(&semafori[resource_id]);
    if(ret == -1) handle_error("process_resource: errore sem_wait");	 
    ...
    ret = sem_post(&semafori[resource_id]);
    if(ret == -1) handle_error("process_resource: errore sem_post");
...
void* connection_handler(void* arg) {
    ...
    while (1) {
        ...
        do {
            ...
            if (ret == -1) {
                ...
                handle_error("connection_handler: errore in recv");
            }
            bytes_read += ret;
        ...
        while (bytes_sent < msg_len) {
            ...
            if (ret == -1) {
                if (errno == EINTR) continue;
                handle_error("connection_handler: errore in send");
            }
            bytes_sent += ret;
        }
    }
     ...
     */
    if (close(socket_desc) == -1) handle_error("connection_handler: errore in close");
...
int main(int argc, char* argv[]) {	  	
    ...
    for (int i = 0; i < N; i++) {
        ...
        if(ret == -1) handle_error("main: errore in sem_init");
    }
    ...
    while (1) { 
         ...
         **/
        handler_args_t* args = calloc(1, sizeof(handler_args_t));
        args->socket_desc = client_desc;
        args->client_addr = (struct sockaddr_in*) client_addr;
        args->client_id = client_id;

        pid_t pid = fork();
        if (pid == -1) handle_error("main: errore in fork");
        else if (pid == 0) {
            close(socket_desc);
        ...
        if (ret != 0) handle_error("main: errore in pthread_create");
    ...
    for (int i = 0; i < N; i++) {
        ...
        if(ret == -1) handle_error("main: errore in sem_destroy");
    }
