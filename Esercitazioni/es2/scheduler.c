...
void* client(void* arg_ptr) {
    ...
    // sem_wait
    if (sem_wait(semaphore) != 0) perror("errore sem_wait\n");
    ...
    // sem_post
    if (sem_post(semaphore) != 0) perror("errore sem_post\n");
...
int main(int argc, char* argv[]) {
    ...
    semaphore = malloc(sizeof(sem_t)); // we allocate a sem_t object on the heap

    // initialize the semaphore
    if (sem_init(semaphore, 0, NUM_RESOURCES) != 0) perror("errore sem_init\n");
    ...
    if (sem_destroy(semaphore) != 0) perror("errore sem_destroy\n");
