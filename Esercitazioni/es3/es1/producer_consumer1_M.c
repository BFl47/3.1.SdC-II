...
void* processTransactions(void* x) {
    ...
    while (args->numOps > 0) {
        ...
        if (sem_wait(&cs) != 0) handle_error("errore sem_wait cs\n");
        ...
        if (sem_post(&cs) != 0) handle_error("errore sem_post cs\n");
...
int main(int argc, char* argv[]) {
    ...
    if (sem_init(&cs, 0, 1) != 0) handle_error("errore sem_init cs\n");
    ...
    if (sem_destroy(&cs) != 0) handle_error("errore sem_destroy cs\n");
