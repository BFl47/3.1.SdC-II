...
void* performTransactions(void* x) {
    ...
    while (args->numOps > 0) {
        ...
        if (sem_wait(&empty) != 0) handle_error("errore sem_wait empty\n");
        if (sem_wait(&cs1) != 0) handle_error("errore sem_wait cs\n");
        ...
        if (sem_post(&cs1) != 0) handle_error("errore sem_post cs\n");
        if (sem_post(&full) != 0) handle_error("errore sem_post full\n");
    }
...
void* processTransactions(void* x) {
    ...
    while (args->numOps > 0) {
        if (sem_wait(&full) != 0) handle_error("errore sem_wait full\n");
        if (sem_wait(&cs2) != 0) handle_error("errore sem_wait cs\n");
        ...
        if (sem_post(&cs2) != 0) handle_error("errore sem_post cs\n");
        if (sem_post(&empty) != 0) handle_error("errore sem_post empty\n");
    }
...
int main(int argc, char* argv[]) {
    ...
    if (sem_init(&empty, 0, BUFFER_SIZE) != 0) handle_error("errore sem_init empty\n");
    if (sem_init(&full, 0, 0) != 0) handle_error("errore sem_init full\n");
    if (sem_init(&cs1, 0, 1) != 0) handle_error("errore sem_init cs\n");
    if (sem_init(&cs2, 0, 1) != 0) handle_error("errore sem_init cs\n");
    ...
    // destroy semaphores
    if (sem_destroy(&empty) != 0) handle_error("errore sem_destroy empty\n");
    if (sem_destroy(&full) != 0) handle_error("errore sem_destroy full\n");
    if (sem_destroy(&cs1) != 0) handle_error("errore sem_destroy cs\n");
    if (sem_destroy(&cs2) != 0) handle_error("errore sem_destroy cs\n");
