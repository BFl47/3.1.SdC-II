...
int main(int argc, char* argv[]) {
    ...
    if (socket_desc == -1) handle_error("client: error creating socket");
    ...
    if (ret == -1) handle_error("client: error connecting to server");
    ...
    do {
        ...
        if (ret == -1) {
            ...
            handle_error("client: error receiving welcome message");
        }
        recv_bytes++;
...
//        buf[--msg_len] = '\0'; // remove '\n' from the end of the message
         ...
         */
        while ((bytes_sent = send(socket_desc, buf, msg_len, 0)) < 0) {
            if (errno == EINTR) continue;
            handle_error("client: error sending message");
        }
         ...
         */
        if (bytes_sent == quit_command_len && !memcmp(buf, quit_command, bytes_sent)) break;
        ...
        do {
            ...
            if (ret == -1) {
                if (errno == EINTR) continue;
                handle_error("client: error receiving message");
            }
            recv_bytes++;
        } while (buf[recv_bytes-1] != '\n');
    ...
    if (ret == -1) handle_error("client: error closing socket");
