...
void openFIFO() {
    ...
    if (fifo == -1) handle_error("openFIFO: open fifo");
...
static void closeFIFO() {
     ...
     * - Destroy the fifo
     * */
    if (close(fifo) == -1) handle_error("closeFIFO: close");
    if (unlink(FIFO_NAME) == -1) handle_error("closeFIFO: unlink");
}
...
int readValue(int * value) {
     ...
     **/
    while (1) {
        ret = read(fifo, value, len);
        if (ret == sizeof(int)) break;
        if (ret == 0) handle_error("readValue: fifo chiusa inaspettatamente");
