...
void initMemory() {
    ...
    if (ftruncate(fd_shm, sizeof(struct shared_memory))) handle_error("initMemory: ftruncate");
...
void closeMemory() {
    ...
    if (shm_unlink(SH_MEM_NAME)) handle_error("closeMemory: shm_unlink");
...
void produce(int id, int numOps) {
    ...
    while (numOps > 0) {
         ...
         */
        while ((myshm_ptr->write_index + 1) % BUFFER_SIZE == myshm_ptr->read_index);        // wait that the consumer has read something
